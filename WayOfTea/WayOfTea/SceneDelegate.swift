...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        if UserDefaults.standard.bool(forKey: "IsUserViewedBanner") {
            navcontroller = UINavigationController(rootViewController: RegViewController())
        }else {
            // TODO: configure for launch
            //            navcontroller = UINavigationController(rootViewController: BannerViewController())
            navcontroller = UINavigationController(rootViewController: RegViewController())
        }
        
        
        //        let navController = UINavigationController(rootViewController: RegViewController())
        let isSigned = UserDefaults.standard.bool(forKey: "IsSignedIn")
        window.rootViewController = isSigned ? configureTabBar() : navcontroller
        self.window = window
